...
select empNome , empSalario , empComision , empFillos , empFillos*30 as "PagaExtra" , empSalario+ifnull(empComision,0)+"PagaExtra" as "SueldoTotal" from empregado where empFillos >= 3 order by empNome;
select empNome from empregado where empNome like "MORA%" order by empNome;
select empNome , empFillos+2 as "Invitaciones" , empFillos as "Regalos" from empregado where empNome between "A" and "M" and empFillos > 0;
...
select empNome , empFillos , empComision , 1.15*ifnull(empComision,0) as "ImpNueComision" from empregado where empFillos > 2 order by empNome;