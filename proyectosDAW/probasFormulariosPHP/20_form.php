    ...
    <div class="container-fluid">
		...
		<div class="row mb-5">
        	...
        	<div class="col-md-8 col-lg-9">
                ...
                //Si se envia o formulario tramitámolo
                if (isset($_REQUEST["envio"]) && $_REQUEST["envio"] == 1 ) {
                    
                    $res = "";
                    $temp = recolle("temperatura");
                    $unidades = recolle("unidades");
                    
                    $tempOk = false;
                    $unidadesOk = false;
                    
                    /*if ($temp == "") {
                        echo "Non inseriches ningún valor na temperatura";
                    } else*/ if (!filter_var($temp, FILTER_VALIDATE_FLOAT) === false) {
                        echo "A temperatura non é númerica";
                    } else if ($temp < -273.15 && $unidades == "c") {
                        echo "Temperatura incorrecta en ºC. Demasiado pequena";
                    } else if ($temp < -459.67 && $unidades == "f") {
                        echo "Temperatura incorrecta en ºF. Demasiado pequena";
                    } else if ($temp > 10000) {
                        echo "Temperatura demasiado grande";
                    } else {
                        $tempOk = true;
                    }
                    
                    if ($unidades == "") {
                        echo "Non introduciches as unidades";
                    } else if ($unidades != "f" && $unidades != "c") {
                        echo "Valores non válidos para unidades";
                    } else {
                        $unidadesOk = true;
                    }
                    
                    if ($tempOk && $unidadesOk) {
                        if ($unidades == "c") {
                            $res = "A temperatura $temp en ºC equivale a " . round(1.8 * $temp + 32, 2) . " ºF";
                        } else {
                            $res = "A temperatura $temp en ºF equivale a " . round(($temp - 32) / 1.8, 2) . " ºC";
                        }
                    ...
                    <input class="form-control" type="number" id="temperatura" name="temperatura" value="0" step="0.01" min="-273.15" max="10000" placeholder="Introduce unha temperatura" required/>
                    
                    <label for="operacion">Unidades</label><br>
                    <input type="radio" id="unidades1" name="unidades" value="c" checked/> <label for="unidades1"> ºC</label>
                    <input type="radio" id="unidades2" name="unidades" value="f"/> <label for="unidades2"> ºF</label>
