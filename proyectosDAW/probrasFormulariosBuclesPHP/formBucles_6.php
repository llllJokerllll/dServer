		...
		<div class="row mb-5">
        	...
        	<div class="col-md-8 col-lg-9">
                ...
                if (isset($_REQUEST["envio"]) && $_REQUEST["envio"] == "1") {
                    ...
                    if (filter_var($filas, FILTER_VALIDATE_INT, ["options" => ["min_range" => 0, "max_range" => 100] ]) === false) {
                        $error .= "<br>Número de filas NO válido";
                    }
                    if (filter_var($columnas, FILTER_VALIDATE_INT, ["options" => ["min_range" => 0, "max_range" => 100] ]) === false) {
                        $error .= "<br>Número de columnas NO válido";
                    }
                    if (filter_var($altura, FILTER_VALIDATE_INT, ["options" => ["min_range" => 40, "max_range" => 100] ]) === false) {
                        $error .= "<br>Número de altura NO válido";
                    }
                    if (filter_var($anchura, FILTER_VALIDATE_INT, ["options" => ["min_range" => 30, "max_range" => 100] ]) === false) {
                        $error .= "<br>Número de anchura NO válido";
                    }
                    
                    if ($error == "") {
                        echo "<table class=\"multiplicacion\">";
                        print "     <tr style=\"height:".$altura."px; width:".$anchura."px;\">\n";
                        print "       <th style=\"height:".$altura."px; width:".$anchura."px;\">X</th>";
                        
                        for ($i = 1; $i <= $filas; $i++) {
                            echo "<th style=\"height:".$altura."px; width:".$anchura."px;\">$i</th>";
                        }
                        echo "</tr>";
                        
                        for ($j = 1; $j <= $columnas; $j++) {
                            echo "<tr style=\"height:".$altura."px; width:".$anchura."px;\">\n";
                            echo "<th style=\"height:".$altura."px; width:".$anchura."px;\">$j</th>";
                            for ($k = 1; $k <= $filas; $k++) {
                                if ($j != $k) {
                                    echo "<td style=\"height:".$altura."px; width:".$anchura."px;\">" . ($j * $k) . "</td>";
                                } else {
                                    echo "<td style=\"height:".$altura."px; width:".$anchura."px; background-color: blue;\">" . ($j * $k) . "</td>";
                                }
                ...
                if (!isset($_REQUEST["envio"]) || $_REQUEST["envio"] != "1" || $error != "") {
                    if ($error !="") {
                ...
                ?>
                    <form class="<?= $error != "" ? 'was-validated':''?>" action="<?= $_SERVER["PHP_SELF"]?>" method="get">
                    	<div class="form-group">
                    		...
                    		<input class="form-control" name="filas" id="filas" type="number" min="0" max="100" value="<?= $filas?>" required>
                    		<div class="valid-feedback">Correcto!</div>
                    		<div class="invalid-feedback">Debes introducir un número entre 0 e 100</div>
                    	</div>
                    	<div class="form-group">
                    		<label for="columnas">Número de columnas</label>
                    		<input class="form-control" name="columnas" id="columnas" type="number" min="0" max="100" value="<?= $columnas?>" required>
                    		<div class="valid-feedback">Correcto!</div>
                    		<div class="invalid-feedback">Debes introducir un número entre 0 e 100</div>
                    	</div>
                    	<div class="form-group">
                    		<label for="altura">Altura das filas en px</label>
                    		<input class="form-control" name="altura" id="altura" type="number" min="40" max="100" value="<?= $altura?>" required>
                    		<div class="valid-feedback">Correcto!</div>
                    		<div class="invalid-feedback">Debes introducir un número entre 40 e 100</div>
                    	</div>
                    	<div class="form-group">
                    		<label for="anchura">Anchura das filas en px</label>
                    		<input class="form-control" name="anchura" id="anchura" type="number" min="30" max="100" value="<?= $anchura?>" required>
                    		<div class="valid-feedback">Correcto!</div>
                    		<div class="invalid-feedback">Debes introducir un número entre 30 e 100</div>
                    	</div>
