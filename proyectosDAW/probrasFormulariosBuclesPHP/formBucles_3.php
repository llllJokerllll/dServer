		...
		<div class="row mb-5">
        	...
        	<div class="col-md-8 col-lg-9">
                ...
                if (isset($_REQUEST["envio"]) && $_REQUEST["envio"] == "1") {
                    ...
                    $num = recolle("numero");
                    if (filter_var($num, FILTER_VALIDATE_INT, ["options" => ["min_range" => 3, "max_range" => 99] ]) === false) {
                        $error .= "<br>Número NO válido";
                    }
                    if ($error == "") {
                        $texto = "El resultado del factorial de  $num! = ";
                        for ($i = $num; $i > 0; $i--) {
                            $res *= $i;
                            if ($i != 1) {
                                $texto .= $i . " x ";
                ...
                ?>
                    <form class="<?= $error != "" ? 'was-validated':''?>" action="<?= $_SERVER["PHP_SELF"]?>" method="get">
                    	<div class="form-group">
                    		...
                    		<input class="form-control" name="numero" id="numero" type="number" min="3" max="99" value="<?= $num?>" required>
